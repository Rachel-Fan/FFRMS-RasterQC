...
# Purpose:     This tool is developed to automate raster QC checklist on FFRMS FVA riverine rasters. 
#              All STARR II PTS Zone3 partners are authorized to use this tool for riverine raster QC checks. 
...
def compareExtent(raster0, raster1, raster2, raster3,tempFolder, outputFolder): #Function to compare the extent of 00FVA, 01FVA, 02 FVA and 03FVA 
    ...
    dissolveFva1_0 = os.path.join(outputFolder, "diffFva1_0.shp")
    arcpy.management.Dissolve(clipFva1_0, dissolveFva1_0)
    ...
    dissolveFva0_1 = os.path.join(outputFolder, "diffFva0_1.shp")
    arcpy.management.Dissolve(clipFva0_1, dissolveFva0_1)

    # Get the count of features in the shapefile
    feature_count = int(arcpy.GetCount_management(dissolveFva0_1).getOutput(0))

    # Check if there are any records
    if feature_count > 0:
        diff0_1_sts = "Fail! See " + dissolveFva0_1 + " in Output folder for details. "
        ...
        diff0_1_sts = "Pass"
        print("Pass. FFRMS FVA 1 raster extent is greater than WSE raster extent. See diffFva1_0.shp in Output folder for details. ")

    clipFva2_1 = os.path.join(tempFolder, "clipFva2_1.shp")
    arcpy.analysis.Erase(polyFva2, polyFva1, clipFva2_1)
    dissolveFva2_1 = os.path.join(outputFolder, "diffFva2_1.shp")
    arcpy.management.Dissolve(clipFva2_1, dissolveFva2_1)

    #detect if lower freeboard values have larger extent by reversely erasing. Warning message is added if it occurs.
    clipFva1_2 = os.path.join(tempFolder, "clipFva1_2.shp")
    arcpy.analysis.Erase(polyFva1, polyFva2, clipFva1_2)
    dissolveFva1_2 = os.path.join(outputFolder, "diffFva1_2.shp")
    arcpy.management.Dissolve(clipFva1_2, dissolveFva1_2)

    # Get the count of features in the shapefile
    feature_count1 = int(arcpy.GetCount_management(dissolveFva1_2).getOutput(0))

    # Check if there are any records
    if feature_count > 0:
        #Define a parameter to pass this "Pass or Fail" value out of the function, and use it in Function createReport
        diff1_2_sts = "Fail! See " + dissolveFva1_2 + " in Output folder for details. "
        ...
        diff1_2_sts = "Pass"
        print("Pass. FFRMS FVA 2 raster extent is greater than FFRMS FVA 1 raster extent. See diffFva3_2.shp in Output folder for details. ")

    clipFva3_2 = os.path.join(tempFolder, "clipFva3_2.shp")
    arcpy.analysis.Erase(polyFva3, polyFva2, clipFva3_2)
    dissolveFva3_2 = os.path.join(outputFolder, "diffFva3_2.shp")
    arcpy.management.Dissolve(clipFva3_2, dissolveFva3_2)

    #detect if lower freeboard values have larger extent by reversely erasing. Warning message is added if it occurs.
    clipFva2_3 = os.path.join(tempFolder, "clipFva2_3.shp")
    arcpy.analysis.Erase(polyFva2, polyFva3, clipFva2_3)
    dissolveFva2_3 = os.path.join(outputFolder, "diffFva2_3.shp")
    arcpy.management.Dissolve(clipFva2_3, dissolveFva2_3)

    # Get the count of features in the shapefile
    feature_count2 = int(arcpy.GetCount_management(dissolveFva2_3).getOutput(0))

    # Check if there are any records
    if feature_count > 0:
        #Define a parameter to pass this "Pass or Fail" value out of the function, and use it in Function createReport
        diff2_3_sts = "Fail! See " + dissolveFva2_3 + " in Output folder for details. "
        ...
        diff2_3_sts = "Pass"
        print("Pass. FFRMS FVA 3 raster extent is greater than FFRMS FVA 2 raster extent. See diffFva3_2.shp in Output folder for details. ")
...
def compareExtent02(raster0, raster02, tempFolder, outputFolder):
    ...
    dissolveFva0_02 = os.path.join(outputFolder, "diffFva0_02.shp")
    arcpy.management.Dissolve(clipFva0_02, dissolveFva0_02)

    #detect if lower freeboard values have larger extent by reversely erasing. Warning message is added if it occurs.
    clipFva02_0 = os.path.join(tempFolder, "clipFva02_0.shp")
    arcpy.analysis.Erase(polyFva02, polyFva0, clipFva02_0)
    dissolveFva02_0 = os.path.join(outputFolder, "diffFva02_0.shp")
    arcpy.management.Dissolve(clipFva02_0, dissolveFva02_0)

    # Get the count of features in the shapefile
    feature_count = int(arcpy.GetCount_management(dissolveFva02_0).getOutput(0))

    # Check if there are any records
    if feature_count > 0:
        diff02_0_sts = "Fail! See " + dissolveFva02_0 + " in Output folder for details. "
        #Define a parameter to pass this "Pass or Fail" value out of the function, and use it in Function createReport
        print("Warning! FFRMS FVA00 raster extent is less than 0.2 PCT raster extent. See diffFva02_0.shp in Output folder for details. ")
    else:
        diff02_0_sts = "Pass"
        print("Pass. FFRMS FVA00 raster extent is greater than 0.2 PCT raster extent. See diffFva0_02.shp in Output folder for details. ")
...
try:
    ...
    raster02 = config['FFRNS 0.2% ACF raster']['Value']
    ...
    try:
        ...
        raster0_properties.extend(("00FVA vs 01FVA", diff0_1_sts, celldiff1_0_sts, "TBD"))
        ...
        raster3_properties.extend(("02PCT vs 00FVA", diff02_0_sts, celldiff0_02_sts, ""))
        raster02_properties.extend(("","", "", ""))
